...
use rex::{rex_kprobe, rex_printk, Result, rex_map};

const TARGET_MAJOR: u32 = 245;
...
#[rex_map]
static sensor_data: RexHashMap<u32, u64> = RexHashMap::new(1, 0);
...
fn check_inode(obj: &kprobe, inode_ptr: usize) -> Option<()> {
    ...
    obj.bpf_probe_read_kernel(&mut i_mode, (inode_ptr + I_MODE_OFFSET) as *const ()).ok()?;
    if (i_mode & S_IFMT) != S_IFCHR {
        return None;
    }
    let mut i_rdev: u32 = 0;
    obj.bpf_probe_read_kernel(&mut i_rdev, (inode_ptr + I_RDEV_OFFSET) as *const ()).ok()?;
...
pub fn rex_sensor_open(obj: &kprobe, ctx: &mut PtRegs) -> Result {
    ...
    if obj.bpf_probe_read_kernel(&mut dentry_ptr, (path_ptr + PATH_DENTRY_OFFSET) as *const ()).is_err() {
        return Ok(0);
    }
    if dentry_ptr == 0 {
        return Ok(0);
    ...
    if obj.bpf_probe_read_kernel(&mut inode_ptr, (dentry_ptr + DENTRY_INODE_OFFSET) as *const ()).is_err() {
        return Ok(0);
    }
...
pub fn rex_sensor_read(obj: &kprobe, ctx: &mut PtRegs) -> Result {
    ...
    let mut inode_ptr: usize = 0;
    if obj.bpf_probe_read_kernel(&mut inode_ptr, (file_ptr + F_INODE_OFFSET) as *const ()).is_err() {
        return Ok(0);
    }
    if check_inode(obj, inode_ptr).is_none() {
        return Ok(0);
